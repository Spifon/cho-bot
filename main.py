...
async def cmd_music(message):
    ...
    if len(words) < 2:
        ...
        return
    query = words[1].strip()
    # КОРОТКАЯ ССЫЛКА - просто заменяем пробелы на +
...
async def on_message(message):
    ...
    try:
        ...
        chat_history[chat_id].append({"role": "user", "content": message.text})        chat_history[chat_id].append({"role": "assistant", "content": ans})
