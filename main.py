...
async def cmd_img(message):    words = message.text.split(" ", 1)
    if len(words) < 2:
    ...
    try:
        ...
        img_url = f"https://image.pollinations.ai/prompt/{prompt}?width=1024&height=1024&seed={seed}&nologo=true"
        print(f"DEBUG: img_url = {img_url}")
        await message.answer_photo(photo=img_url, caption=f"Готово: {prompt}")
    except Exception as e:
        print(f"DEBUG: img error = {e}")
        await message.answer(f"Не вышло! Ошибка: {str(e)}")

async def cmd_music(message):
    await message.answer("Музыка пока не работает, извини!")
...
async def on_message(message):
    ...
    try:
        ...
        if len(chat_history[chat_id]) > 6:
            chat_history[chat_id] = chat_history[chat_id][-6:]        await message.answer(ans)
    except Exception as e:
