...
async def cmd_img(message):
    ...
    try:
        ...
        # Pollinations.ai - бесплатный API
        img_url = f"https://image.pollinations.ai/prompt/{encoded_prompt}?width=1024&height=1024&nologo=true&seed=42"
        
        # Отправляем картинку
        await message.answer_photo(photo=img_url, caption=f"Вот что получилось: {prompt}")
        
    except Exception as e:
        await message.answer(f"Блядь, не вышло! Ошибка: {str(e)}")
...
Пример: /img красивый закат
"""
    await message.answer(help_text)

@dp.message()
async def on_message(message):
    me = await bot.me    bot_id = me.id
